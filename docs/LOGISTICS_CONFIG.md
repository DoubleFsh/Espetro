...
3. 玩家右击已建成的弹药箱补充当前职业装备变体的实际缺口；Radio 弹药不足完整费用时整次取消。
...
载具不扫描世界容器，也不自动补给。补给载具在 Radio 或本方原部署点附近使用 F 轮盘
装卸物资；战斗载具在本方原部署点或建造出的载具补给站附近装卸弹药。每次转移量、间隔
和作用半径由 `config/espetro/fortifications.json` 配置。所有部署载具的初始载荷都是空的。
