...
```json
{
  ...
  "damage": {
    "explosion_damage_ratio": 0.1,
    "projectile_hit_damage_ratio": 0.1
  },
  "fortifications": [
    {
      "id": "ammo_crate",
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `explosion_damage_ratio` | 0.1 | 爆炸对工事的伤害比例；原伤害 × 该值，最低 0 |
| `projectile_hit_damage_ratio` | 0.1 | superb 小口径炮弹直接命中的伤害比例；原伤害 × 该值，最低 0 |

铁铲拆除不受这两个比例影响，仍按 `remove_per_hit` 固定扣除。
...
6. 建成后可以继续用铁铲右键降低完整度；只有进度归零时整座工事才消失。爆炸和小口径炮弹
   命中会按结构方块数量计算基础伤害，再乘以 `damage` 中的比例；随后左键可按进度逐步恢复缺失方块。
